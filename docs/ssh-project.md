...
- Agent and selected skill Markdown are bounded immutable local snapshots, not directory grants or stock `/skill:` commands. `read` defaults to the remote project. `scope: "local-resource"` reads only the exact path of a selected Markdown snapshot; it does not expose helpers, assets, neighboring files or credentials. Relative links do not grant access. Text `write` pins the destination with `pwd -P` under the bound project, refuses a leaf or parent-directory symlink (including dangling parents), creates missing dest components with non-following `mkdir` from that directory, and opens `./basename` with `dd oflag=nofollow` (`O_NOFOLLOW`) so a concurrent remote Bash swap of a dest component or leaf to an outward symlink cannot redirect the write. If remote `dd` lacks `oflag=nofollow`, write stages into an exclusive directory and replaces the leaf with `mv -T` or `ln -n`/`ln -h` (no-dereference). If none of those opens are available, write fails closed rather than following a pathname. This remains execution routing, **not a sandbox**: concurrent remote Bash can still mutate the project, but this write does not follow a swapped-outward symlink.
