...
Controls the duration, in milliseconds, for model exclusions. The default is `86400000` (24 hours), and the maximum is `8000000000000000` so generated expiry timestamps remain valid JavaScript dates. The extension applies this value when it starts or reloads. A lower configured value shortens active cached exclusions from their original `recordedAt`; it never extends an existing expiry. Authentication-related exclusions are ignored when Pi's `auth.json` was modified after the exclusion was recorded; other exclusion types are unaffected. Launches also warn when a candidate is skipped, including the cached reason and expiry. `PI_MODEL_EXCLUSIONS_PATH` changes the exclusion-store path but does not change this TTL. When every configured candidate is excluded only for a transient transport or provider-availability reason, one process may re-probe a single candidate after creating an exclusive claim file next to the store (`<exclusions-path>.recovery-probe.<candidate>.json`). The claim exists because the exclusion store is a shared cache, not a lock, and foreground plus detached runners would otherwise each send a real request. Auth, quota, rate-limit, request-shape, explicit-model, configuration, and model-scope failures stay fail-closed. A successful probe clears only that matching exclusion; claim or persist cleanup never discards a completed child.
