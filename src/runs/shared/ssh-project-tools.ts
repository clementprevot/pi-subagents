...
function remoteWriteScript(file: string, encoded: string): string {
	return [
		...
		"test -n \"$box\"",
		`printf '%s' ${sshQuote(encoded)} | base64 -d | dd of="$box/p" 2>/dev/null`,
		"test -f \"$box/p\" && test ! -L \"$box/p\"",
		"ok=",
		"if mv -T -- \"$box/p\" \"./$base\" 2>/dev/null; then ok=1; elif ln -fh -- \"$box/p\" \"./$base\" 2>/dev/null; then ok=1; elif ln -fn -- \"$box/p\" \"./$base\" 2>/dev/null; then ok=1; fi",
		"rm -rf -- \"$box\"",
		"test -n \"$ok\"",
		"fi",
