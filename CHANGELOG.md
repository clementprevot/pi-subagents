...
- Keep host-discovered tool names that extensions provide: core tool slots resolve by name whatever their source (extensions that re-register a core tool shadowed it with an extension source, so child tool plans pruned every requested tool and foreground scout/reviewer launches failed), and extension tool names declared in agent `tools:` lists are kept when the child loads the providing extension (ambient on runner launches when the source is rediscoverable from the runner's own settings, or listed in the agent's `extensions`). Restrictive launches (capability ceilings, explicit extension lists, foreground ambient isolation) still fail closed. (#2132, #2133, #2134, #2135)
## [0.67.0] - 2026-09-10
